...
* Function prototypes
****************************************************************************************/
static boot_bool Rs232ReceiveByte(boot_int8u *data);
static void     Rs232TransmitByte(boot_int8u data);
...
**
****************************************************************************************/
void Rs232TransmitPacket(boot_int8u *data, boot_int8u len)
{
  boot_int16u data_index;
  #if (BOOT_COM_RS232_CS_TYPE == 1)
  boot_int8u csByte = len;
  #endif
...
**
****************************************************************************************/
boot_bool Rs232ReceivePacket(boot_int8u *data, boot_int8u *len)
{
  /* one extra for length and two extra for possibly configured checksum byte(s). */
  static boot_int8u xcpCtoReqPacket[BOOT_COM_RS232_RX_MAX_DATA+3];
  static boot_int8u xcpCtoRxLength;
  static boot_bool  xcpCtoRxInProgress = boot_FALSE;
  static boot_int32u xcpCtoRxStartTime = 0;
  #if (BOOT_COM_RS232_CS_TYPE == 1)
  boot_int8u  csLen = 1;
  boot_int8u  csByte;
  boot_int16u csIdx;
  ...
  if (xcpCtoRxInProgress == boot_FALSE)
  {
    /* store the message length when received */
    if (Rs232ReceiveByte(&xcpCtoReqPacket[0]) == boot_TRUE)
    {
  ...
  else
  {
    ...
    if (Rs232ReceiveByte(&xcpCtoReqPacket[xcpCtoRxLength+1]) == boot_TRUE)
    {
      ...
      if (xcpCtoRxLength == (xcpCtoReqPacket[0] + csLen))
      {
        ...
        CpuMemCopy((boot_int32u)data, (boot_int32u)&xcpCtoReqPacket[1], xcpCtoRxLength);
