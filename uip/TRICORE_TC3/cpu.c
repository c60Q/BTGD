...
extern boot_bool CpuUserProgramStartHook(void);
#endif
...
* Function prototypes
****************************************************************************************/
static void CpuEnableUncorrectableEccErrorTrap(boot_bool enable);
...
void CpuInit(void)
{
  ...
  CpuEnableUncorrectableEccErrorTrap(boot_FALSE);
} /*** end of CpuInit ***/
...
void CpuStartUserProgram(void)
{
  ...
  if (NvmVerifyChecksum() == boot_FALSE)
  {
...
#if (BOOT_CPU_USER_PROGRAM_START_HOOK > 0)
  /* invoke callback */
  if (CpuUserProgramStartHook() == boot_FALSE)
  {
  #if (BOOT_COM_DEFERRED_INIT_ENABLE > 0) && (BOOT_COM_ENABLE > 0)
    /* bootloader will stay active so perform deferred initialization to make sure
     * the communication interface that were not yet initialized are now initialized.
  ...
  CpuEnableUncorrectableEccErrorTrap(boot_TRUE);
...
**
****************************************************************************************/
void CpuMemCopy(boot_addr dest, boot_addr src, boot_int16u len)
{
  boot_int8u *from, *to;
...
**
****************************************************************************************/
void CpuMemSet(boot_addr dest, boot_int8u value, boot_int16u len)
{
  boot_int8u *to;
...
**            set.
** \param     enable boot_TRUE to enable generation of the CPU trap, boot_FALSE to disable.
** \return    none.
**
****************************************************************************************/
static void CpuEnableUncorrectableEccErrorTrap(boot_bool enable)
{
  ...
  boot_int16u         password;
  Ifx_CPU *          cpu;
  boot_int8u          maskUEccVal;
