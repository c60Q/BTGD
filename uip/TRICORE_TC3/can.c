...
{
  boot_int8u tseg1;                                    /**< CAN time segment 1          */
  boot_int8u tseg2;                                    /**< CAN time segment 2          */
} tCanBusTiming;
...
** \return    boot_TRUE if the CAN bustiming register values were found, boot_FALSE
**            otherwise.
**
****************************************************************************************/
static boot_bool CanGetSpeedConfig(boot_int16u baud, boot_int16u *prescaler,
                                  boot_int8u *tseg1, boot_int8u *tseg2)
{
  boot_int8u  cnt;
  boot_int32u canClockFreqkHz;

  /* determine the clock frequency that sources the CAN controller and is used for the
   * baudrate generation.
   */
  canClockFreqkHz = ((boot_int32u)IfxCan_getModuleFrequency()) / 1000;
...
void CanInit(void)
{
  boot_int16u prescaler = 0;
  boot_int8u  tseg1 = 0, tseg2 = 0;
  boot_int32u rxMsgId = BOOT_COM_CAN_RX_MSG_ID;
  ...
  if (CanGetSpeedConfig(BOOT_COM_CAN_BAUDRATE/1000, &prescaler, &tseg1, &tseg2) == boot_FALSE)
  {
  ...
  if ((rxMsgId & 0x80000000) == 0)
  {
    ...
    rxFilter0 = IfxCan_Node_getStandardFilterElementAddress((boot_addr)MCMCAN_MODULE->RAM,
                                                            CAN_RX_FILTER_STD_MESSAGE_RAM_BASE_ADDR,
                                                            0);
  ...
  else
  {
    ...
    rxFilter0 = IfxCan_Node_getExtendedFilterElementAddress((boot_addr)MCMCAN_MODULE->RAM,
                                                            CAN_RX_FILTER_EXT_MESSAGE_RAM_BASE_ADDR,
                                                            0);
    /* link the filter to the dedicated reception buffer 0. */
...
**
****************************************************************************************/
void CanTransmitPacket(boot_int8u *data, boot_int8u len)
{
  boot_int32u txMsgId = BOOT_COM_CAN_TX_MSG_ID;
  boot_int32u timeout;
  Ifx_CAN_TXMSG * txBuf0;
  uint32 txMsgData[2];

  /* validate parameters. */
  ASSERT_RT((data != boot_NULL) && (len <= 8));

  /* obtain message RAM pointer for the dedicated transmit buffer 0. */
  txBuf0 = IfxCan_Node_getTxBufferElementAddress(MCMCAN_NODE,
                                                 (boot_addr)MCMCAN_MODULE->RAM,
...
** \return    boot_TRUE is a packet was received, boot_FALSE otherwise.
**
****************************************************************************************/
boot_bool CanReceivePacket(boot_int8u *data, boot_int8u *len)
{
  ...
  if (IfxCan_Node_isRxBufferNewDataUpdated(MCMCAN_NODE, IfxCan_RxBufferId_0) == TRUE)
  {
     ...
     */
    *len = (boot_int8u)IfxCan_Node_getDataLengthCode(rxBuf0);
    ...
    if (*len <= 8)
    {
      ...
      data[0] = (boot_int8u)rxMsgData[0];
      data[1] = (boot_int8u)(rxMsgData[0] >> 8);
      data[2] = (boot_int8u)(rxMsgData[0] >> 16);
      data[3] = (boot_int8u)(rxMsgData[0] >> 24);
      data[4] = (boot_int8u)rxMsgData[1];
      data[5] = (boot_int8u)(rxMsgData[1] >> 8);
      data[6] = (boot_int8u)(rxMsgData[1] >> 16);
      data[7] = (boot_int8u)(rxMsgData[1] >> 24);
