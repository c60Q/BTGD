...
* Include files
****************************************************************************************/
#include "boot_types.h"                                /* bootloader generic header          */
...
typedef struct
{
  boot_addr   sector_start;                       /**< sector start address             */
  boot_int32u sector_size;                        /**< sector size in bytes             */
  boot_int8u  sector_num;                         /**< sector number                    */
} tFlashSector;
...
{
  boot_addr  base_addr;
  boot_int8u data[FLASH_WRITE_BLOCK_SIZE];
} tFlashBlockInfo;
...
extern boot_bool FlashCryptoDecryptDataHook(boot_addr address, boot_int8u * data, 
                                           boot_int32u size);
#endif
...
* Function prototypes
****************************************************************************************/
static boot_bool  FlashInitBlock(tFlashBlockInfo *block, boot_addr address);
static tFlashBlockInfo *FlashSwitchBlock(tFlashBlockInfo *block, boot_addr base_addr);
static boot_bool  FlashAddToBlock(tFlashBlockInfo *block, boot_addr address,
                                 boot_int8u *data, boot_int32u len);
static boot_bool  FlashWriteBlock(tFlashBlockInfo *block);
static boot_bool  FlashEraseSectors(boot_int8u first_sector_idx, 
                                   boot_int8u last_sector_idx);
static boot_int8u FlashGetSectorIdx(boot_addr address);
static boot_bool  FlashEraseLogicalSectors(boot_addr log_sector_base_addr,
                                          boot_int16u num_log_sectors);
static boot_bool  FlashWritePage(boot_addr page_base_addr, boot_int8u const * page_data);
...
static const tFlashSector flashLayout[] =
{
  ...
  /* { 0xA0008000, 0x004000,   2},         flash sector   2 - reserved for bootloader  */
...
** \return    boot_TRUE if successful, boot_FALSE otherwise.
**
****************************************************************************************/
boot_bool FlashWrite(boot_addr addr, boot_int32u len, boot_int8u *data)
{
  boot_bool result = boot_TRUE;
  boot_addr base_addr;
...
{
  boot_bool   result = boot_TRUE;
  boot_int32u signature_checksum = 0;
  ...
  if (bootBlockInfo.base_addr != FLASH_INVALID_ADDRESS)
  {
    ...
    if (FlashCryptoDecryptDataHook(bootBlockInfo.base_addr, bootBlockInfo.data, 
                                   FLASH_WRITE_BLOCK_SIZE) == boot_FALSE)
    {
      result = boot_FALSE;
    }
#endif
    ...
    if (result == boot_TRUE)
    {
       ...
       */
      signature_checksum += *((boot_int32u *)(&bootBlockInfo.data[0+0x00]));
      signature_checksum += *((boot_int32u *)(&bootBlockInfo.data[0+0x04]));
      signature_checksum += *((boot_int32u *)(&bootBlockInfo.data[0+0x08]));
      signature_checksum += *((boot_int32u *)(&bootBlockInfo.data[0+0x0C]));
      signature_checksum += *((boot_int32u *)(&bootBlockInfo.data[0+0x10]));
      signature_checksum += *((boot_int32u *)(&bootBlockInfo.data[0+0x14]));
      signature_checksum += *((boot_int32u *)(&bootBlockInfo.data[0+0x18]));
      signature_checksum  = ~signature_checksum; /* one's complement */

      /* write the checksum */
      result = FlashWrite(flashLayout[0].sector_start+BOOT_FLASH_VECTOR_TABLE_CS_OFFSET,
                          sizeof(boot_addr), (boot_int8u *)&signature_checksum);
    }
  }
...
boot_bool FlashVerifyChecksum(void)
{
  boot_bool   result = boot_TRUE;
  boot_int32u signature_checksum = 0;
  boot_int32u signature_checksum_rom;

  /* verify the checksum based on how it was written by FlashWriteChecksum(). */
  signature_checksum += *((boot_int32u *)(flashLayout[0].sector_start));
  signature_checksum += *((boot_int32u *)(flashLayout[0].sector_start+0x04));
  signature_checksum += *((boot_int32u *)(flashLayout[0].sector_start+0x08));
  signature_checksum += *((boot_int32u *)(flashLayout[0].sector_start+0x0C));
  signature_checksum += *((boot_int32u *)(flashLayout[0].sector_start+0x10));
  signature_checksum += *((boot_int32u *)(flashLayout[0].sector_start+0x14));
  signature_checksum += *((boot_int32u *)(flashLayout[0].sector_start+0x18));
  signature_checksum = ~signature_checksum; /* one's complement */

  /* read the checksum value from flash that was writtin by the bootloader at the end
   * of the last firmware update
   */
  signature_checksum_rom = *((boot_int32u *)(flashLayout[0].sector_start+BOOT_FLASH_VECTOR_TABLE_CS_OFFSET));
...
boot_bool FlashDone(void)
{
  ...
  if (bootBlockInfo.base_addr != FLASH_INVALID_ADDRESS)
  {
    if (FlashWriteBlock(&bootBlockInfo) == boot_FALSE)
    {
  ...
  if (result == boot_TRUE)
  {
    ...
    {
      if (FlashWriteBlock(&blockInfo) == boot_FALSE)
      {
        /* update the result value to flag the error */
        result = boot_FALSE;
      }
...
static boot_bool FlashInitBlock(tFlashBlockInfo *block, boot_addr address)
{
  ...
  if (result == boot_TRUE)
  {
    ...
    if (block->base_addr != address)
    {
      ...
      CpuMemCopy((boot_addr)block->data, address, FLASH_WRITE_BLOCK_SIZE);
    }
  }
...
static tFlashBlockInfo *FlashSwitchBlock(tFlashBlockInfo *block, boot_addr base_addr)
{
  tFlashBlockInfo * result = boot_NULL;
  ...
  else
  {
    ...
    if (FlashWriteBlock(block) == boot_FALSE)
    {
  ...
  if (result != boot_NULL)
  {
    ...
    if (FlashInitBlock(block, base_addr) == boot_FALSE)
    {
      /* invalidate the result value to flag the error */
      result = boot_NULL;
    }
                                ...
                                boot_int8u *data, boot_int32u len)
{
  ...
  if (block->base_addr == FLASH_INVALID_ADDRESS)
  {
    ...
    if (FlashInitBlock(block, current_base_addr) == boot_FALSE)
    {
      result = boot_FALSE;
    }
  }
  ...
  if (result == boot_TRUE)
  {
    ...
    do
    {
      ...
      CopService();
      /* buffer overflow? */
      if ((boot_addr)(dst-&(block->data[0])) >= FLASH_WRITE_BLOCK_SIZE)
      {
        /* need to switch to a new block, so program the current one and init the next */
...
static boot_bool FlashWriteBlock(tFlashBlockInfo *block)
{
  boot_bool           result = boot_TRUE;
  boot_addr           page_addr;
  boot_int8u        * page_data;
  boot_int32u         page_cnt;
  ...
  if (result == boot_TRUE)
  {
    ...
    for (page_cnt=0; page_cnt < (FLASH_WRITE_BLOCK_SIZE/IFXFLASH_PFLASH_PAGE_LENGTH); page_cnt++)
    {
      ...
      if (FlashWritePage(page_addr, page_data) == boot_FALSE)
      {
...
**
****************************************************************************************/
static boot_bool FlashEraseSectors(boot_int8u first_sector_idx, boot_int8u last_sector_idx)
{
  boot_bool   result = boot_TRUE;
  boot_int8u  sectorIdx;
  boot_addr   sectorBaseAddr;
  boot_int32u sectorSize;
  boot_addr   logSectorBaseAddr;
  boot_int16u numOfLogSectorsToErase;
  boot_bool   logSectorEraseResult;
  ...
  if (result == boot_TRUE)
  {
    ...
    for (sectorIdx = first_sector_idx; sectorIdx <= last_sector_idx; sectorIdx++)
    {
      ...
      numOfLogSectorsToErase = (boot_int16u)(sectorSize / FLASH_ERASE_BLOCK_SIZE);
      ...
      if(logSectorEraseResult == boot_FALSE)
      {
...
** \return    boot_TRUE if the logical sectors were successfully erased, boot_FALSE
**            otherwise.
**
****************************************************************************************/
BLT_RAM_FUNC_BEGIN
...
{
  boot_bool   result = boot_TRUE;
  boot_int16u endInitSafetyPassword;
  boot_bool   alreadyErased = boot_FALSE;
...
** \return    boot_TRUE if the page was successfully programmed, boot_FALSE otherwise.
**
****************************************************************************************/
BLT_RAM_FUNC_BEGIN
static boot_bool FlashWritePage(boot_addr page_base_addr, boot_int8u const * page_data)
{
  boot_bool           result = boot_TRUE;
  boot_int16u         endInitSafetyPassword;
  boot_int32u         dword_cnt;
  boot_int32u const * page_data_ptr;
  boot_int32u const * word_flash_ptr;
  boot_int32u const * word_data_ptr;
  boot_int32u         word_cnt;
  ...
  page_data_ptr = (boot_int32u const *)page_data;
  /* clear all error and status flags. */
  IfxFlash_clearStatus(0);
  ...
  for (dword_cnt=0; dword_cnt<(IFXFLASH_PFLASH_PAGE_LENGTH/(sizeof(boot_int32u) * 2)); dword_cnt++)
  {
  ...
  if (result == boot_TRUE)
  {
    ...
    word_flash_ptr = (boot_int32u const *)page_base_addr;
    word_data_ptr  = (boot_int32u const *)page_data;
    /* verify that the written data is actually there, one word at a time. */
    for (word_cnt=0; word_cnt<(IFXFLASH_PFLASH_PAGE_LENGTH/sizeof(boot_int32u)); word_cnt++)
    {
...
static boot_int8u FlashGetSectorIdx(boot_addr address)
{
  boot_int8u result = FLASH_INVALID_SECTOR_IDX;
  boot_int8u sectorIdx;
