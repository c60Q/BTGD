/************************************************************************************//**
* \file         Source/TRICORE_TC3/mbrtu.c
 ...
 */
static boot_int16u mbRtuT3_5Ticks;


/****************************************************************************************
* Function prototypes
****************************************************************************************/
static boot_bool   MbRtuReceiveByte(boot_int8u *data);
static void       MbRtuTransmitByte(boot_int8u data, boot_bool end_of_packet);
...
void MbRtuInit(void)
{
  boot_int16u startTimeTicks;
  boot_int16u deltaTimeTicks;
  boot_int16u currentTimeTicks;
  boot_int8u  rxDummy;
  ...
  ASSERT_CT((BOOT_COM_MBRTU_CHANNEL_INDEX == 0)  ||
            ...
            (BOOT_COM_MBRTU_CHANNEL_INDEX == 8)  ||
            (BOOT_COM_MBRTU_CHANNEL_INDEX == 9)  ||
            (BOOT_COM_MBRTU_CHANNEL_INDEX == 10) ||
            (BOOT_COM_MBRTU_CHANNEL_INDEX == 11));
  ...
  else
  {
     ...
     */
    mbRtuT3_5Ticks = (boot_int16u)(((3850000UL + (BOOT_COM_MBRTU_BAUDRATE - 1U)) /
  ...
  MbRtuDriverOutputControlHook(boot_FALSE);
  ...
  do
  {
    ...
    if (MbRtuReceiveByte(&rxDummy) == boot_TRUE)
    {
...
**
****************************************************************************************/
void MbRtuTransmitPacket(boot_int8u *data, boot_int8u len)
{
  boot_int16u data_index;
  boot_int16u checksum;
  boot_bool endOfPacket = boot_FALSE;
  /* Made static to lower stack load and +5 for Modbus RTU packet overhead. */
  static boot_int8u txPacket[BOOT_COM_MBRTU_TX_MAX_DATA + 5];
  ...
  CpuMemCopy((boot_int32u)&txPacket[3], (boot_int32u)data, len);
  ...
  txPacket[len + 3] = (boot_int8u)(checksum & 0xff);
  txPacket[len + 4] = (boot_int8u)(checksum  >> 8);
  ...
  MbRtuDriverOutputControlHook(boot_TRUE);
  MbRtuDelay(BOOT_COM_MBRTU_DRIVER_OUTPUT_ENABLE_DELAY_US);
...
** \return    boot_TRUE if a packet was received, boot_FALSE otherwise.
**
****************************************************************************************/
boot_bool MbRtuReceivePacket(boot_int8u *data, boot_int8u *len)
{
  boot_bool result = boot_FALSE;
  boot_int8u rxByte;
  boot_int16u currentTimeTicks;
  boot_int16u deltaTimeTicks;
  boot_int16u checksumCalculated;
  boot_int16u checksumReceived;
  /* Made static to lower stack load and +5 for Modbus RTU packet overhead. */
  static boot_int8u rxPacket[BOOT_COM_MBRTU_RX_MAX_DATA + 5];
  static boot_int8u rxLength = 0;
  static boot_bool packetRxInProgress = boot_FALSE;
  static boot_int16u lastRxByteTimeTicks = 0;

  /* get the current value of the free running counter. */
  currentTimeTicks = MbRtuFreeRunningCounterGet();

  /* check for a newly received byte. */
  if (MbRtuReceiveByte(&rxByte) == boot_TRUE)
  {
    ...
    if (packetRxInProgress == boot_FALSE)
    {
      /* initialize the reception of a new packet. */
      rxLength = 0;
      packetRxInProgress = boot_TRUE;
    }
        ...
        if (checksumCalculated == checksumReceived)
        {
               ...
               (rxPacket[1] == BOOT_COM_MBRTU_FCT_CODE_USER_XCP) )
          {
            ...
            if (rxPacket[2] == (rxLength - 5))
            {
              ...
              CpuMemCopy((boot_int32u)data, (boot_int32u)&rxPacket[3], rxLength - 5);
...
** \return    boot_TRUE if a byte was received, boot_FALSE otherwise.
**
****************************************************************************************/
static boot_bool MbRtuReceiveByte(boot_int8u *data)
{
...
** \param     end_of_packet boot_TRUE if this is the last byte of the packet, boot_FALSE
**            otherwise.
** \return    none.
**
****************************************************************************************/
...
**
****************************************************************************************/
boot_int16u MbRtuFreeRunningCounterGet(void)
{
  static boot_bool initialized = boot_FALSE;
  static boot_int32u counts_per_ten_us = 0;
  static boot_int32u free_running_counter_last = 0;
  static boot_int16u ten_us_tick_counter = 0;
  boot_int32u free_running_counter_now;
  boot_int32u delta_counts;
  boot_int32u ten_us_ticks;

  /* perform one-time initialization.*/
  if (initialized == boot_FALSE)
  {
    /* set the flag to make sure that this part only runs once. */
    initialized = boot_TRUE;
